...
export const useRemindersDomain = () => {
  ...
  const viewConfigs = {
    scheduleWorkspace: {
      ...
      description:
        "Track time-sensitive health tasks, follow-up notifications, and critical patient alerts.",
      icon: "📆",
    },
    emptyWorkspace: {
      title: "All Caught Up!",
      description:
        "You have no pending reminders or scheduled alerts right now. Enjoy the clear calendar!",
      icon: "🎉",
    },
    creationForm: {
      title: "Schedule New Reminder",
      description:
        "Set an alert window, frequency parameters, and task instructions for an upcoming objective.",
      icon: "🔔",
    },
  };
  ...
  return {
    ...
    flags: {
      ...
      isUpdating: Boolean(updatePending && updateVars?.reminder_id),
      isDeleting: Boolean(deletePending && deletingId),
      isCompleting: Boolean(completePending && completingId),
      isActionInFlight:
        createPending || updatePending || deletePending || completePending,
    },

    actions: {
      scheduleReminder: (body: any) => createMutation(body),
      modifyReminder: (reminder_id: string, body: any) =>
        updateMutation({ reminder_id, body }),
